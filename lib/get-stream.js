module.exports = {
  ...
  fn: function (inputs,exits) {
    ...
    stream.on('response', function(httpResponse){
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // The `request` package seems to (but doesn't actually) start the stream flowing
      // when a 'response' listener is bound.  This is kinda weird, and definitely not
      // what we want.  There might be a way to normalize that by using `.pause()`--
      // for more info & context, see:
      ...
      //
      // …but after trying many different variations, it became clear that this is not
      // going to work without a deeper fix in `request`.  So in the mean time, you
      // CANNOT ALWAYS SAFELY USE `.pipe()` on the stream returned from this method!!
      // It's not that data is actually flowing... that part seems to be fine.  It's
      // just that there is a too-aggressive `on('pipe')` handler in the source code
      // preventing it from working, even though it should.  So instead of `.pipe()`,
      // in userland code, you should use a hand-rolled streams1-compatible approach.
      // (See `machinepack-strings/from-stream` and `machinepack-fs/write-stream` for
      // a couple of examples.)
      //
      // FUTURE: Solve this in a better way!
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
